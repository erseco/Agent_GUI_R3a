	...
	switch(id){
		...
		case SELECTMAP_BTN_ID:
			...
			select_map_glui=GLUI_Master.create_glui("New Game Setting",0,100,100);
				...
				map_list=new GLUI_FileBrowser(select_map_glui,"",GLUI_PANEL_NONE,SELECTMAP_SELECT_ID,control_cb);
				...
				#ifdef _WIN32
				//Hay que mirar como se le deberÌa de pasar eso, para no tener que forzar
				//a hacerlo en filebrowser.cpp y asÌ no tocar el cÛdigo de GLUI
				//NOTA: De todas formas habÌa un problema en glui_internal.h, asÌ que
				//hay que compilar siempre nuestra propia librerÌa glui
		...
		case DISPLAY_BTN_ID:
			if(true){
				...
				for(long long t=atol(display_step);t>0 && current_time<life_time;--t){
					...
					//En unix y en windows, el sleep funciona de manera diferente
					#ifdef _WIN32
					Sleep(s);
					#else
					usleep(s*1000);
...
int GUI::startDraw(int argc,char **argv){
	...
	score_glui=GLUI_Master.create_glui_subwindow(main_window,GLUI_SUBWINDOW_BOTTOM);
		...
		new GLUI_Column(score_glui,false);
		new GLUI_Column(score_glui,false);
		new GLUI_StaticText(score_glui,"");//•H™≈•’¶Ê∞µ§¿πj 
	...
	main_glui=GLUI_Master.create_glui_subwindow(main_window,GLUI_SUBWINDOW_RIGHT);
		new GLUI_StaticText(main_glui,"");//•H™≈•’¶Ê∞µ§¿πj 
		...
		new GLUI_StaticText(main_glui,"");//•H™≈•’¶Ê∞µ§¿πj 
		new GLUI_StaticText(main_glui,"");//•H™≈•’¶Ê∞µ§¿πj 
		new GLUI_StaticText(main_glui,"DisplayOption");//•H™≈•’¶Ê∞µ§¿πj 
		...
		new GLUI_StaticText(main_glui,"");//•H™≈•’¶Ê∞µ§¿πj 
		new GLUI_StaticText(main_glui,"");//•H™≈•’¶Ê∞µ§¿πj 
		new GLUI_Button(main_glui,"Quit",0,(GLUI_Update_CB)exit );//QUIT´ˆ∂sµ≤ßÙµ{¶° 
...
void GUI::doOneStep(){
	if(current_time<life_time){
		...
		++current_time;
		if(current_time==life_time){//§@Ω¸µ≤ßÙ 
			do_one_step_btn->disable();
			display_btn->disable();
			do_one_run_btn->disable();
			...
			if(current_run<total_runs){//¨Oß_¡Ÿ¶≥§U§@Ω¸ 
				next_run_btn->enable();
			}
			else{
				do_all_run_btn->disable();
